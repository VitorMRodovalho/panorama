...
| **Backups** | Spatie-style app-level backups + DB dump + object-store copy; **single-DB restore drill** ([`docs/runbooks/restore.md`](../runbooks/restore.md)) + executable companion (`scripts/restore-drill.sh`) | Point-in-time recovery via WAL shipping, cross-region DR, **fleet-orchestrated restore drills** with per-tenant evidence capture |
